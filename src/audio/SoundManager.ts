...
export class SoundManager {
  ...
  public playLineClear(lines: number, combo = 0) {
    ...
    freqs.slice(0, lines).forEach((freq, idx) => {
      ...
      const pitchedFreq = freq * Math.pow(1.05, combo);
      osc.frequency.setValueAtTime(pitchedFreq, this.audioCtx.currentTime + idx * 0.06);

      gain.gain.setValueAtTime(0.2, this.audioCtx.currentTime + idx * 0.06);
      gain.gain.exponentialRampToValueAtTime(0.001, this.audioCtx.currentTime + idx * 0.06 + 0.15);
  ...
  public playGameOver() {
    ...
    notes.forEach((freq, idx) => {
      ...
      gain.gain.exponentialRampToValueAtTime(0.001, this.audioCtx.currentTime + idx * 0.12 + 0.2);
